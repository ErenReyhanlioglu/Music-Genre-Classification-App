...
def load_and_preprocess_data(file_path, target_shape=(210, 210)):
    ...
    for i in range(num_chunks):
        ...
        mel_spectrogram = image.resize(np.expand_dims(mel_spectrogram, axis=-1), target_shape)
...
#Prediction Page
    ...
    if test_mp3 is not None:
            filepath = 'Test Data/'+test_mp3.name
    ...
    if(st.button("Predict")):
      with st.spinner("Please Wait.."):       
        ...
        st.balloons()
        label = ['rock', 'reggae', 'pop', 'metal', 'jazz', 'hiphop', 'disco', 'country', 'classical', 'blues']
